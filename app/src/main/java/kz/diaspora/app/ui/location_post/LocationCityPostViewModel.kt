...
@HiltViewModel
class LocationCityPostViewModel @Inject constructor(
    ...
    init {
        if (prefsImpl.getCountryPostId() != null)
            getCityList(prefsImpl.getCountryPostId())
    }
    ...
    fun saveCity(cityId: String, city: String) {
        prefsImpl.setCityPostId(cityId)
        prefsImpl.setCityPost(city)
        launchIO {
            isRefreshing.postValue(true)
            val data = baseCloudRepository.saveCityFrom(Integer.parseInt(cityId))
            when (data) {
                is ResultWrapper.Error -> error.postValue(data)
                is ResultWrapper.Success -> {
