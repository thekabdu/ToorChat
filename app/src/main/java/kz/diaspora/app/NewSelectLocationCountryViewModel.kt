...
@HiltViewModel
class NewSelectLocationCountryViewModel @Inject constructor(
    ...
    private val baseCloudRepository: BaseCloudRepository
        ...
        fun getSearchList(searchText: String) {
            launchIO {
                ...
                for (i in countryList.value?.indices!!) {
                    ...
                    if (country.country_name.toLowerCase(Locale.ROOT).contains(searchText.toLowerCase(Locale.ROOT))) {
        ...
        fun saveCountry(countryId: String, country: String) {
            prefsImpl.setCountryId(countryId)
            prefsImpl.setCountry(country)
        }
